...
const taskSchema = new mongoose.Schema({
  ...
  embedding: {
    // NEW: Field to store the vector embedding from the Python service
...
async function sendForEmbedding(taskId, title, description) {
  try {
    ...
    if (!response.ok) {
      ...
      console.error(
        `Error from Python embedding service: ${errorData.message}`
      );
    } else {
    ...
    console.error(
      `Failed to connect to Python embedding service for task ${taskId}:`,
      error
    );
  }
}
...
app.post("/signup", async (req, res) => {
  try {
    ...
    const signupToken = jwt.sign(signupPayload, JWT_SECRET, {
      expiresIn: "7d",
    });
    res
      .status(201)
...
app.get("/tasks", async (req, res) => {
  try {
    ...
    if (useSemantic && originalQuery) {
      ...
      try {
        const pythonResponse = await fetch(
          `${PYTHON_SERVICE_URL}/search_semantic`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ query: originalQuery }),
        ...
        if (!pythonResponse.ok) {
          const errorData = await pythonResponse.json();
          throw new Error(
            `Python semantic search service error: ${errorData.message}`
          );
        }
        ...
        console.log(
          `Semantic search returned ${semanticTaskIds.size} relevant task IDs.`
        );
      } catch (pythonError) {
        console.error(
          "Error calling Python semantic search service:",
          pythonError
        );
    ...
    if (useContains && originalQuery) {
      ...
      console.log(
        `"Contains" search returned ${containsTaskIds.size} relevant task IDs.`
      );
    }
    ...
    if (useSemantic && useContains) {
      ...
      combinedTaskIds = Array.from(
        new Set([...semanticTaskIds, ...containsTaskIds])
      );
      console.log(
        `Combined semantic and contains search yielded ${combinedTaskIds.length} unique task IDs.`
      );
    } else if (useSemantic) {
    ...
    if (originalQuery && combinedTaskIds.length > 0) {
      ...
      finalQuery._id = {
        $in: combinedTaskIds.map((id) => new mongoose.Types.ObjectId(id)),
      };
      ...
      originalQuery &&
      combinedTaskIds.length === 0 &&
      (useSemantic || useContains)
    ) {
...
app.delete("/tasks", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: "All scheduled tasks deleted successfully" });
  } catch (error) {
