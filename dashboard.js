// dashboard.js original - Script for dashboard card-based UI with backend integration
...
function createTaskCard(task) {
  ...
  cardFooterDiv.appendChild(dueDateP);
  ...
  cardDiv.dataset.archived = taskCompleted.toString();
  ...
  updateCardUI(cardDiv, taskCompleted, taskCompleted);
...
async function handleTaskActions(e) {
  ...
  // Handle Complete/Restore button click
  if (e.target.getAttribute("data-action") === "archive") {
    ...
    if (card) {
      ...
      try {
        await handleCompleteToggle(card);
      } finally {
...
// Complete toggle state tracking
let archiveToggleInProgress = new Set();
let lastCompleteClick = 0; // Global archive click throttle

// --- Handle Complete Toggle ---
async function handleCompleteToggle(card) {
  const now = Date.now();
  if (now - lastCompleteClick < 1000) return;
  lastCompleteClick = now;
  ...
  try {
    ...
    card.dataset.archived = newCompletedState.toString();
    card.dataset.completed = newCompletedState.toString();

    updateCardUI(card, newCompletedState, newCompletedState);
...
function initDashboard() {
  ...
  setTimeout(() => {
    document.querySelectorAll(".task-card").forEach((card) => {
      const isCompleted = card.dataset.archived === "true";
...
function updateCardUI(card, isCompleted, isCompleted) {
  ...
  const actualCompletedState = isCompleted ?? card.dataset.archived === "true";
  // const actualCompletedState = isCompleted ?? card.dataset.completed === "true";

  // Update datasets for consistency
  card.dataset.archived = actualCompletedState.toString();
  ...
  if (actualCompletedState || actualCompletedState) {
    ...
    if (archiveIcon) updateCompleteIcon(archiveIcon, true);

    // Add archived badge if needed
    if (actualCompletedState) {
    ...
    if (archiveIcon) updateCompleteIcon(archiveIcon, false);
  }
}

// Helper function to update archive icon state
function updateCompleteIcon(archiveIcon, isCompleted) {
...
function populateTaskDetailsModal(taskData) {
  ...
  document.getElementById("archiveTaskBtn").onclick = function () {
    ...
    if (taskCard) handleCompleteToggle(taskCard);
    bootstrap.Modal.getInstance(
      document.getElementById("taskDetailsModal")
    ).hide();
  };
...
function getFileTypeInfo(fileExtension) {
  const fileTypeInfo = {
    ...
    zip: "Complete File - Requires extraction software",
    rar: "Complete File - Requires extraction software",
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (taskDetailsModalElement) {
        // Add an event listener for when the taskDetailsModal is completely hidden
        taskDetailsModalElement.addEventListener('hidden.bs.modal', function () {
            // Ensure the body's modal-open class is removed
            // This is a safeguard; Bootstrap usually removes it.
            document.body.classList.remove('modal-open');

            // Ensure all modal backdrops are removed
            // This is also a safeguard, especially useful if backdrops accumulate.
